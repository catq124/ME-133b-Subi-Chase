...
class VisualGrid:
    ...
    def __init__(self, rows, cols):
        ...
        for row in range(self.rows+1):
            self.ax.axhline(row, lw=1, color='lightgray', zorder=1)
        for col in range(self.cols+1):
            self.ax.axvline(col, lw=1, color='lightgray', zorder=1)
        
        # Draw the grid boarders
        self.ax.axhline(0, lw=3, color="black", zorder=1)
        self.ax.axhline(self.rows, lw=3, color="black", zorder=1)
        self.ax.axvline(0, lw=3, color="black", zorder=1)
        self.ax.axvline(self.cols, lw=3, color="black", zorder=1)
